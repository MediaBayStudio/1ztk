...
function enqueue_style( $style_name, $widths ) {
  ...
  if ( is_string( $widths ) ) {
    if ( $style_name === 'hover' ) {
      wp_enqueue_style( "{$style_name}", $template_directory . "/css/{$style_name}.css", [], null, "(hover) and (min-width:1024px)" );
    } else {
...
add_action( 'wp_enqueue_scripts', function() {
  ...
  wp_register_script( 'jquery', false, ['jquery-core'], null, true, true );
...
// Убираем id и type в тегах script, добавляем нужным атрибут defer
  add_filter( 'script_loader_tag',   function( $html, $handle ) {
    // if ( $handle === 'fancybox.min' && is_page_template( '' ) ) {
...
// Убираем id и type в тегах style
  add_filter( 'style_loader_tag', function( $html, $handle ) {
    ...
    if ( !is_single() && !is_admin() && $handle === 'wp-block-library' || $handle === 'bvi-style' ) {
      return '';
    }
